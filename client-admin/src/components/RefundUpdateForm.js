...
import { editRefund, fetchRefunds } from "../redux/actions/dataActions";
...
export default function RefundUpdateForm(props) {
  ...
  const validate = (fieldValues = values) => {
    let temp = { ...errors };
    if ("paymentId" in fieldValues)
      temp.paymentId = fieldValues.paymentId ? "" : "This field is required.";
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      const refundId = recordForEdit._id;
      const bookingId = recordForEdit.bookingId;

      console.log(refundId);
      console.log(bookingId);

      dispatch(editRefund(refundData, refundId, bookingId));

      // {
      loading === false && dispatch(fetchRefunds());
      ...
      let x = setInterval(function () {
        ...
        if (distance < 0) {
          ...
          editOrAdd(refundData, resetForm);
        }
      }, 1000);
      // }
    }
